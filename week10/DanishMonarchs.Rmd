...
kings1 <- read.csv("data/Kongerækken-csv.csv")
glimpse(kings1)

kings2 <- read_csv("data/Kongerækken-csv.csv")
glimpse(kings2)
class(kings2)

kings3 <- read.csv2("data/Kongerækken-csv.csv")
glimpse(kings3)

kings4 <- read_csv2( "data/Kongerækken-csv.csv")
glimpse(kings4)
class(kings4)
```
Answer: 
...
read.csv():   1 column
read_csv():   1 column
read.csv2():  19 columns
read_csv2():  19 columns
...
kings$Død...13 <- as.numeric(kings$Død...13)
class(kings$Død...13)
```
...
```{r}
avg_duration <- mean(na.omit(kings_new$duration))
avg_duration
```
...
Angeom_smooth()And to submit this rmarkdown, knit it into html. But first, clean up the code chunks, adjust the date, rename the author and change the `eval=FALSE` flag to `eval=TRUE` so your script actually generates an output.
Well done!
