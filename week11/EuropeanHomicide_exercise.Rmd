...
ggplot(data = Western_Europe) + 
  ...
  labs(title = "Long term trend in homicides", 
       x = "Year", 
...
```{r plot-rule-duration, eval = TRUE}
kongerækken <- read_csv2("data/kongerækken-csv.csv")
kongerækken <- mutate(kongerækken, Midtår = `Regering slut`-(`Regering slut`-`Regerings start`)/2)

ggplot(data = kongerækken) +
  geom_smooth(aes(x = Midtår, y = `Regerings år i alt`)) +
  labs(title = "Long term trend in duration of rules of danish kings",
       x = "Year",
       y = "Duration of rule (years)")
```
...
The short answer quite clearly is: yes we are more civilized today. The visualizations indicate a clear downward trend in homicide rates across Western Europe from the Middle Ages to the 20th century. This decline in violence suggests that society has become more peaceful and less violent over time, which could be interpreted as a sign of increased civilization. The reduction in homicide rates reflects broader societal changes, including the establishment of stronger legal systems, improved law enforcement, and more effective conflict resolution mechanisms. These developments have contributed to a more stable and secure environment, where violent crimes are less prevalent. 
Moreover, economic development and urbanization have transformed societies, leading to greater dependence and cooperation among individuals. As communities have become more interconnected, the incentives for violent behavior have decreased. The rise of democratic institutions and the rule of law has also provided citizens with peaceful means to address grievances and seek justice, further reducing the need for violent retribution. 
