...
leaflet() %>%
  ...
  setView(lng = 151.205006, lat = -33.8767231, zoom = 12)              # set the location of the map 
...
leaflet() %>% 
  ...
  addLayersControl(baseGroups = c("Geo","Aerial", "Physical"),          # we are adding layers control to the maps
                   options = layersControlOptions(collapsed = FALSE))   # replace T with F and back and run it
...
# Before youl_dk# Before you can proceed to Task 2, you need to learn about coordinate creation. 
...
# Your brief answer
  # The clustered is better for visibility as the pins does not cover the entire map. 
  # It is however not as easy to see the exact locations of the pins as the unclustered map.
...
# Solution
DANmap_details <- DANmap %>% 
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(places$Description, "<br>", places$Type, "<br>", places$Notes, "<br>", places$`Greatness(1_ok_5_ecstatic)`),
             clusterOptions = markerClusterOptions())
DANmap_details

saveWidget(DANmap_details, "DANmap.html", selfcontained = TRUE)

######################################## CONGRATULATIONS - YOUR ARE DONE :)

# Own dataset (a selection of places to get drunk)
...
Beers_map <- DANmap %>% 
  ...
  addMarkers(
    lng = Beers$Longitude,
    lat = Beers$Latitude,
    popup = paste(Beers$Placename, "<br>", Beers$Description, "<br>", Beers$Type),
    clusterOptions = markerClusterOptions()
  )
Beers_map

saveWidget(Beers_map, "Beersmap.html", selfcontained = TRUE)
